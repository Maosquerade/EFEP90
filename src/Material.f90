...
!	Material history data structure
	...
	integer:: LenHistoryList=0, LenHistoryList_root=0      ! Length of history list
	type(History), TARGET, allocatable:: history_list(:), history_list_root(:)
...
! ---------------------------------------------------------------------
	...
	J2 = 0.5d0*(sd(1)*sd(1)+sd(2)*sd(2)+sd(3)*sd(3))+sd(4)*sd(4)+sd(5)*sd(5)+sd(6)*sd(6)
...
! ---------------------------------------------------------------------
	...
	! ---------------------------------------------------------------------
		...
		if (seqv .GT. sig_y_) then	
			...
			PlaMod = n_jnc_*B_jnc_*epeff_**(n_jnc_-1)
			depeff = (seqv - sig_y_) / (1.5e0*G2 + PlaMod)
			epeff_ = epeff_ + depeff
